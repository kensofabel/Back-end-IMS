...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    // DEBUG: Output the username/email being checked
    file_put_contents('login_debug.txt', "Username input: [$username]\n", FILE_APPEND);

    $sql = "SELECT * FROM users WHERE username='$username' OR email='$username' LIMIT 1";
    file_put_contents('login_debug.txt', "SQL: $sql\n", FILE_APPEND);

    $result = $conn->query($sql);
    file_put_contents('login_debug.txt', "Query num_rows: {$result->num_rows}\n", FILE_APPEND);
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // DEBUG: Log both input and DB password
        file_put_contents('login_debug.txt', "Input password: [$password], DB password: [{$user['password']}]\n", FILE_APPEND);
        ...
        function handleAuthFailure(reason) {
            ...
            switch (reason) {
                ...
                case 'Employee record not found':
                    errorMessage = 'Account not found. Please verify your username/email.';
                    break;
                default:
