...
function showNotFoundConfirmation(message, onConfirm, onCancel) {
    // Stop scanner/camera when confirmation is shown
    ...
    cancelBtn.onclick = function() {
        if (dontShowCheckbox && dontShowCheckbox.checked) window.notFoundConfirmSkip = true;
        cleanup();
        // Restart scanner/camera when cancelling confirmation
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function openInlineAddItemsPanel() {
        ...
        if (backBtn) {
            ...
            newBackBtn.addEventListener('click', function(e) {
                e.preventDefault();
                // If previousTab is 'manual', go to 'scan' (scan flow with autofill), else go to previousTab or fallback to 'scan'
        ...
        try {
            ...
            try {
                ...
                if (playPromise && typeof playPromise.then === 'function') {
                    playPromise.then(() => { startQuaggaScanner(); }).catch((err) => { startQuaggaScanner(); });
                } else {
                    startQuaggaScanner();
                }
            } catch (e) {
    ...
    function startQuaggaScanner() {
        ensureQuaggaLoaded(() => {
            ...
            const MIN_IDENTICAL = 4;
            const MAX_IDENTICAL = 7;
            ...
            .then(data => {
                if (data && data.found) {
                    ...
                    showNotFoundConfirmation(
                        ...
                        function() { openAddItemsPrefill('', barcode, true); }, // true: back button will restart scanner
                        function() {
                            // Always fully reset Quagga and scanner before restarting
                            try {
                                if (window.Quagga && window.Quagga.initialized) {
                                    window.Quagga.stop();
                                    window.Quagga.initialized = false;
                            ...
                            setTimeout(() => { checkAndInitializeScanner(); }, 200);
                        }
                    );
                }
            })
    ...
    function openAddItemsPrefill(sku, barcode) {
        ...
        // Always stop scanner when opening Add Item
        stopScanner();
            ...
            setTimeout(() => {
                ...
                if (backBtn) {
                    ...
                    newBackBtn.addEventListener('click', function(e) {
                        ...
                        // Optionally restart scanner if needed
                        setTimeout(() => { checkAndInitializeScanner(); }, 250);
                    });
                } else {
        ...
        setTimeout(() => {
            ...
            // If opened from scanner (not from confirmation), add a one-time event listener to restart scanning when user goes back/close
            if (fromScanner) {
...
function showTab(tabName, transition = true) {
    ...
    if (!(currentTab === 'addItems' && tabName !== 'addItems')) {
        ...
        if (activePanel) {
            ...
            // Always start scanner when switching to scan tab
            if (tabName === 'scan' && typeof startQuaggaScanner === 'function') {
                startQuaggaScanner();
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Tab switching
    if (scanTabBtn) scanTabBtn.onclick = () => {
        showTab('scan', false);
        if (typeof startQuaggaScanner === 'function') {
            startQuaggaScanner();
        }
    };
    if (manualTabBtn) manualTabBtn.onclick = () => {
        showTab('manual', false);
        if (typeof stopScanner === 'function') {
            stopScanner();
        }
    };
