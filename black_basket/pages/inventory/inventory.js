document.addEventListener('DOMContentLoaded', function () {
    ...
    productsData = products;
        ...
        if (products.length === 0) {
            ...
            tr.innerHTML = `<td colspan="7" style="text-align:center;color:#ff9800;font-size:1.1em;padding:24px 0;background:#222;border-radius:8px;">No products found.</td>`;
            inventoryTableBody.appendChild(tr);
        } else {
            products.forEach(product => {
                const tr = document.createElement('tr');
                const cost = product.cost !== undefined && product.cost !== null ? `₱${parseFloat(product.cost).toFixed(2)}` : '<span class="badge badge-grey">N/A</span>';
                const margin = product.margin !== undefined && product.margin !== null ? `<span class="badge badge-green">${product.margin}</span>` : '<span class="badge badge-grey">N/A</span>';
                let stock;
                if (product.quantity === undefined || product.quantity === null || product.quantity <= 0) {
                    stock = '<span class="badge badge-red">Out of Stock</span>';
                } else {
                    stock = `<span class="badge badge-blue">${product.quantity}</span>`;
                }
                tr.innerHTML = `
                    <td style="font-weight:600;color:#ff9800;">${product.name || 'N/A'}</td>
                    <td style="color:#fff;">${product.category || 'N/A'}</td>
                    <td style="color:#fff;">₱${product.unit_price !== undefined && product.unit_price !== null ? parseFloat(product.unit_price).toFixed(2) : '0.00'}</td>
                    <td>${cost}</td>
                    <td>${margin}</td>
                    <td>${stock}</td>
                    <td style="text-align:center;">
                        <button class="edit-btn" data-id="${product.id}" style="background:#2196f3;color:#fff;border:none;padding:6px 14px;border-radius:4px;margin-right:6px;cursor:pointer;">Edit</button>
                        <button class="delete-btn" data-id="${product.id}" style="background:#f44336;color:#fff;border:none;padding:6px 14px;border-radius:4px;cursor:pointer;">Delete</button>
                    </td>
                `;
                inventoryTableBody.appendChild(tr);
            });
